...
Add <AuthProvider> wrapping all routes. Add <PrivateRoute> component:

if (!isLoggedIn) → <Navigate to="/login" replace />
else → <Outlet />
Wrap protected routes: /cart, /order-history, /ingredients (CRUD pages) Keep /market and /login public.

4. frontend/src/pages/login.page.jsx
Add form: username + password fields + submit button. On submit → login(username, password) from useAuth() → navigate to /market. On error → show error message.
...
Verification
Open /market → no redirect (public)
Open /cart → redirect to /login
Login with valid creds → redirect to /market
Open /cart → accessible
Wait 30min or manually expire token → next API call auto-refreshes silently
Logout → /cart redirects to /login again